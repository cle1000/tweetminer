...
class TweetMiner {
    constructor() {
        ...
        this.stream.on('error', (error) =>  this.stop( `Twitter reports error: ${error}`));
        this.stream.on('end', () => {this.stop("Twitter stream end..")});
        this.stream.on('destroy', () => {this.stop("Twitter stream destroy..")});
